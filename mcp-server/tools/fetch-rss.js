...
async function fetchTopicRSS(topic, maxAgeDays = 7) {
  ...
  for (const source of sources) {
    ...
    if (source.credibility < config.settings.min_credibility) {
      console.log(`  ⏭️  跳过（可信度过低）：${source.name} (${source.credibility})`);
      continue;
    }
...
async function fetchAllRSS(maxAgeDays = 7) {
  const config = await loadRSSConfig();
  const topics = ['ai_programming', 'generative_ai', 'ai_chips', 'quantum_computing', 'robotics'];
  ...
  const totalCount = Object.values(allResults).reduce((sum, items) => sum + items.length, 0);
