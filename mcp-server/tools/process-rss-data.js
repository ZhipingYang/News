...
import { deduplicateBatch, addToHistory, cleanupOldData } from '../utils/deduplicator.js';
...
async function processRSSData(rssDataPath, minCredibility = 0.85) {
  ...
  // 统计
  const total = Object.values(allProcessed).reduce((sum, items) => sum + items.length, 0);
  console.log(`\n✨ 处理完成！共 ${total} 条高质量资讯\n`);
...
async function main() {
  try {
    const rssFile = process.argv[2] || path.join(__dirname, '../data/rss-fetch-2025-11-07.json');
    ...
    await fs.writeFile(outputPath, JSON.stringify({ byDate, processed }, null, 2), 'utf-8');
