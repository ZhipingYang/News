...
async function fetchGitHubTrendingRSS() {
  try {
    ...
    for (const match of itemMatches) {
      ...
      const starsMatch =
        description.match(/(\d+)\s*stars?\s*today/i) ||
...
function evaluateProject(project) {
  ...
  return {
    ...
    reasons,
    category:
      score >= 70 ? "must-include" : score >= 50 ? "recommended" : "optional",
  };
}
...
async function main() {
  ...
  const mustInclude = evaluatedProjects.filter(
    (p) => p.evaluation.category === "must-include"
  );
  const recommended = evaluatedProjects.filter(
    (p) => p.evaluation.category === "recommended"
  );
  ...
  console.log(
    `📋 可选添加 (评分<50): ${
      evaluatedProjects.length - mustInclude.length - recommended.length
    } 个\n`
  );
  ...
  if (mustInclude.length > 0) {
    ...
    mustInclude.forEach((p, i) => {
      ...
      console.log(
        `   ⭐ Stars: ${p.stars.toLocaleString()} | 📈 今日: +${p.starsToday}`
      );
